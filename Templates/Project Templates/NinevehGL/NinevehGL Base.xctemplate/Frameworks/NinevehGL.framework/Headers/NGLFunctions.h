...
/*!
 *					Checks if a file exists at the specified path or at least at NinevehGL Global Path.
 *
 *	@param			named
 *					A NSString (name or path) containing the file's name.
 *
 *	@result			A BOOL representing if the file exists or not.
 */
NGL_API BOOL nglFileExists(NSString *named);
