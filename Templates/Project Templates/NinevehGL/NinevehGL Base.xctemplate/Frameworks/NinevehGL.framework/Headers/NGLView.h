 ...
 *
 *					This class uses the #drawView# method (defined in #NGLViewDelegate#) as its render
 *					cycle. It's called automatically by the NinevehGL core at each render cycle.
 *					The number of calls per second will vary depending on the complexity of the render
 *					you're trying to make. You can define an expected frame rate (fps) by using the
 *					global function #nglGlobalFPS#. By default the frame rate starts at 60.0, that means
 *					the #drawView# method will be called 60 times per second.
 *
 *					The render status (paused or not) works in conjunction with the superview property.
 *					When this view is added as a subview of any other view class, its render will start
 *					(paused = NO). When this view is removed from its superview the render will stop
 *					(paused = YES). This is a security behavior to save performance. There is two ways
 *					to change this bahvior:
 *						- Using the hidden property instead of adding and removing;
 *						- Subclassing this class and overriding the "didMoveTo..." and "removeFrom...".
 *
 *	@see			NGLCamera
 *	@see			NGLViewDelegate
 *	@see			nglGlobalEngine
 *	@see			nglGlobalFPS
 */
@interface NGLView : UIView <NGLViewDelegate>
{
@private
	// States.
	BOOL					_paused;
 ...
 *
 *					<b>IMPORTANT</b>: This property will be automatically define as YES when this view
 *					is added as a subview from any oter view class and it will be set to NO when this view
 *					is removed from its superview. This is a security behavior to save performance and
 *					can't be changed. You can use the hidden property to show it off and keep rendering.
 */
...
- (void) drawingOffscreen;
- (void) drawingFrame;

/*!
 *					No more calls to this method is needed.
 */
- (void) compileCoreEngine DEPRECATED_ATTRIBUTE;
 ...
 *
 *					This method works like a screenshot or snapshot. It doesn't produce new renders,
 ...
 *					#NGLTexture#.
 *
 *					So, if you have made changes until the last call to #drawView# method, those changes
 *					will not appear in the output of this method.
 *
