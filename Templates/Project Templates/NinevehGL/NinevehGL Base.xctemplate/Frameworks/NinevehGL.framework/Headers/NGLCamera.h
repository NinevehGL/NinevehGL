...
/*!
 *					Defines zoom for this camera.
 *
 *					The zoom will bring far objects closer, however this method doesn't change any other
 *					property of the camera, that means the far plane will still clipping the objects very
 *					far from the camera and the all other behaviors will still the same.
 *
 *					There is no negative value for this property, because doesn't exist nagative zoom.
 *					If you set a negative value, it will be turned into 0.0 (zero). However there is an
 *					important behaviour about the zoom:
 *
 *						- The original zoom is 1.0, which produces the original image size.
 *						- Zoom values > 1.0 will increase the zoom.
 *						- Zoom values < 1.0 and > 0.0 will produce smaller results than the original.
 *						- Zoom values = 0.0 will produce no image.
 *
 ...
 *
 *					The angle of view can also be used as a kind of camera zoom. By increasing this
 *					property the camera will zoom out and by increasing it the camera will zoom in.
 *					This zoom approach is not the same used by the #zoom# property. That property
 *					was made specifically to work with real zoom, changing the angleView is just
 *					a fake zoom.
 *
 ...
 *
 *					One single mesh can be attached to multiple cameras. If a mesh is already attached
 *					to this camera, new attempts to put it in the same camera will be ignored.
 *
 *	@param			mesh
