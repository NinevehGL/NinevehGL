...
typedef enum
{
	NGLTextureQualityLow		= 0x01,
	NGLTextureQualityMedium		= 0x02,
	NGLTextureQualityHigh		= 0x03,
} NGLTextureQuality;
...
/*!
 *					Defines the visible wireframe modes for an arbitrary mesh. The wireframe is composed
 *					by colored lines and it is only for visual purposes.
 *
 *	@see			NGLWireframe
 *	@see			nglGlobalWireMode
 *	
 *	@var			NGLWireModeNone
 *					Negates any other kind of wireframe.
 *	
 *	@var			NGLWireModeOnly
 *					Shows only the wireframe, hidding the mesh it self.
 *	
 *	@var			NGLWireModeGeometry
 *					Shows the wireframe above the mesh.
 *	
 *	@var			NGLWireModeVolume
 *					Shows the volume box's wireframe.
 *	
 *	@var			NGLWireModeBoundingBox
 *					Shows the bounding box's wireframe.
 */
typedef enum
{
	NGLWireModeNone				= 0x01,
	NGLWireModeOnly				= 0x02,
	NGLWireModeGeometry			= 0x04,
	NGLWireModeVolume			= 0x08,
	NGLWireModeBoundingBox		= 0x10,
} NGLWireMode;
...
/*!
 *					The NinevehGL global wireframe mode property.
 *
 *	@see			NGLWireMode
 *	@see			nglGlobalWireMode
 */
NGL_API NGLWireMode nglDefaultWireMode;
 ...
 *					made on back faces (NGLFrontFaceCCW, NGLCullFaceBack).
 *
 *					Just as any other property of the NinevehGL Global API, this will affect the whole
 *					application. This function must be followed by a call to #nglGlobalFlush#.
 *	
...
/*!
 *					Defines the global wireframe mode.
 *
 *					Although every #NGLMesh# has its own wireframe property, you can specify a global
 *					wireframe mode to be used in place. If the global wireframe is set to NGL_NULL, that
 *					means the local wireframe properties will be used, but if you set a valid wireframe
 *					to this function, then it will be used to any #NGLMesh#. This property affects all
 *					meshes, including the existing ones.
 *
 *					Just as any other property of the NinevehGL Global API, this will affect the whole
 *					application.
 *	
 *	@param			mode
 *					The NGLWireMode representing the desired wireframe mode or NGL_NULL to
 *					negate this global property.
 *
 *	@see			NGLWireMode
 *	@see			NGLMesh
 */
NGL_API void nglGlobalWireMode(NGLWireMode mode);
