...
/*!
 *					Very fast way to check if a memory addres is valid or not. By simply calling this
 *					method and respecting its return the commum error BAD_ADDRSS can be avoided and
 *					gracefully result in a warning instead of a crash.
 *
 *	@param			p
 *					A memory pointer to check.
 */
 ...
 */
NGL_API BOOL nglPointerIsObjc(void *pointer);

/*!
 *					Checks if a pointer is valid for a specific kind of class.
 *
 *					To check the validation against any kind of classes, use the #nglPointerIsObjc#.
 *	
 *	@param			pointer
 *					A C pointer to check.
 *
 ...
 *	@result			A BOOL indicating if the pointer is valid or not.
 */
NGL_API BOOL nglPointerIsObjcClass(void *pointer, Class aClass);
 ...
 *	@result			A BOOL indicating if the pointer is valid or not.
 */
NGL_API BOOL nglPointerIsObjcProtocol(void *pointer, Protocol *aProtocol);
 ...
 *	@result			A BOOL indicating if the pointer is valid or not.
 */
NGL_API BOOL nglPointerIsObjcSelector(void *pointer, SEL selector);
 ...
 */
@property (nonatomic, readonly) NGL_ARC_UNSAFE id *itemsPointer;
