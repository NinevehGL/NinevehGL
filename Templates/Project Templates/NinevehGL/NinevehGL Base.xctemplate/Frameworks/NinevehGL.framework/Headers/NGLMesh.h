...
#import "NGLShaderMulti.h"
#import "NGLWireframe.h"
#import "NGLCamera.h"
...
/*!
 *					The mesh "Updating Process" happens everytime the mesh deep structure (vertices,
 *					texture coordinates, normals, tangent, bitangent, etc) changes. Notice that
 *					animated meshes, animated materials and animated surfaces don't represent an
 *					"Updating Process".
 *
 *					Actually this process just happens when a new mesh's structure must be resent to
 *					the core of NinevehGL. Also when you explicit updates the OpenGL engine of NinevehGL,
 *					all the meshes will be updated.
 *
 *					The "Updating Process" is an asynchronous task and can use the mesh's binary cache.
 *
 *	@param			parsing
 *					The parsing object.
 *
 *	@see			NGLParsing
 */
- (void) meshUpdatingDidFinish:(NGLParsing)parsing;
 ...
 *	@see			NGLMeshDelegate
 */
@interface NGLMesh : NGLObject3D <NGLGestureRecognizer>
{
@private
	// Matrices
	NGLmat4					_mvpMatrix;
	...
	// Effects
	NGLWireframe			*_wireframe;
	id <NGLMaterial>		_material;
	id <NGLShader>			_shader;
...
/*!
 *					The wireframe object defines the helper visual lines that will be rendered within this
 *					mesh. This object holds all the information about the wireframe's lines.
 *
 *					The default value is nil.
 */
@property (nonatomic, retain) NGLWireframe *wireframe;
...
/*!
 *					This property can be set to any NGLShader. This include the basic shaders
 *					NGLShader and the multi/sub shaders #NGLShaderMulti#.
 ...
 *	@see			NGLShaderMulti
 */
@property (nonatomic, retain) id <NGLShader> shader;
 ...
 *
 *	@param			named
 *					In NinevehGL the "named" parameter is always related to the NinevehGL Global Path,
 *					you can inform only the file's name or the full path. There are three possible patterns:
 *						- Absolute Path: When informing a full system path, it will be taken as literaly.
 *						- Relative Path: When informing a relative path, it will start in the Global Path.
 *						- File's Name: When informing the file's name, it will search in the Global Path.
 *
 *	@param			dict
 ...
 *
 *	@param			named
 *					In NinevehGL the "named" parameter is always related to the NinevehGL Path API, which 
 *					can receive the file's name or the full path. The full path is related to the device's
 *					file system. If only the file's name is informed, NinevehGL will search for the file
 *					at the global path.
 *
 *	@param			dict
 *					A NSDictionary containing the keys and values to the preset.
...
/*!
 *					Starts the render process with a specific camera. This method should not be called
 *					directly, however, if you really need, it can be called informing a camera and the
 *					mesh will be rendered within the current render cycle.
 *
 *					By calling this method manually the "visible" property will be ignored and the mesh
 ...
 *					its length.
 */
- (void) setIndices:(unsigned int *)indices count:(unsigned int)count;

/*!
 *					Sets the array of structures.
 *
 *					Use this method to create the array of structures and set its length.
 *
 *	@param			structures
 ...
 *					specify its length.
 */
- (void) setStructures:(float *)structures count:(unsigned int)count;
