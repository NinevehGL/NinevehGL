 ...
 *
 *	@var			NGLSLVariable::info
 *					The pointer to an arbitrary info. When the info is an Obj-C class, it'll be internally
 *					retained.
 */
typedef struct
{
	BOOL isDynamic;
	NGL_ARC_UNSAFE NSString *name;
...
/*!
 *					The default empty value for NGLSLVariable.
 */
static const NGLSLVariable kNGLSLVariableEmpty = { NO, nil, 0, 0, 0, 0, NULL, NULL };
...
/*!
 *					Replaces a variable in this library. The variable's name will be as the identifier key
 *					the variable.
 *
 *					All the values will be copied and a new memory block will be allocated. The original 
 *					library and/or values can be deleted with no impact on these new copies.
 ...
 *	@see			NGLSLVariable
 */
//- (void) replaceVariable:(NGLSLVariable)variable;
